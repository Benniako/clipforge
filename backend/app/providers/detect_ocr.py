...
_GENERIC: dict[str, tuple[str, ...]] = {
    ...
    "eliminated": ("eliminated", "knocked", "knockout", "k.o", "ko",
                   ...
                   "team eliminiert", "team eliminated", "eliminated team"),
    "kill": ("double kill", "triple kill", "multi kill", "ultra kill", "rampage",
             "killing spree", "first blood", "headshot", "doppelkill",
             "dreifachkill", "mehrfachkill", "kopfschuss", "getötet",
             "abgeschossen", "eliminiert",
             "getötet von", "kampfbericht", "tödlich", "verursacht",
             "assist"),
    "ace": ("ace", "team ace", "flawless"),
    "clutch": ("clutch", "1v5", "1v4", "1v3", "1v2",
               "runde läuft", "letzte runde", "last round", "letzter spieler",
               "last player standing"),
    "bomb": ("spike planted", "spike defused", "spike platziert",
             "spike entschärft", "spike entscharft",
             "spike-entschärfer", "spike-entscharfer",
             "spike entschärft", "spike entscharft",
             "spike-entschärft", "spike-entscharft",
             "entschärft", "entscharft",
             "bombe gelegt", "bombe entschärft", "bombe entscharft"),
    ...
    "goal": ("goal", "go!", "tor", "tore", "touchdown", "score",
             "auswechslung", "vorlage", "hattrick",
             "elfer", "elfmeter", "abpfiff"),
    "penalty": ("penalty", "red card", "yellow card", "gelbe karte",
                "rote karte", "freistoss", "foul",
...
_PROFILE_EXTRA: dict[str, dict[str, tuple[str, ...]]] = {
    "valorant": {
        ...
        "kill": ("headshot", "kopfschuss", "abgeschossen", "eliminiert",
                 ...
                 "gegner übrig",
                 "kampfbericht", "tödlich", "verursacht", "getötet von",
                 "erhalten"),
        ...
        "defeat": ("runden verloren", "letzte runde", "runde verloren"),
    },
    "cs2": {
...
def sample_frame_times(duration: float, *, every: float = 1.0,
                       max_frames: int = 2000) -> list[float]:
...
def focused_frame_times(duration: float, focus_times: list[float] | None, *,
                        every: float = 1.0, max_frames: int = 2000) -> list[float]:
...
def find_text_events(src_path: str, info: MediaInfo,
    ...
    # Max-density OCR sampling: with EasyOCR GPU at ~0.4-0.5s/frame on the
    # RTX 5060 Ti, 2000 frames adds ~13-17 min. For a 60 min VOD that's every
    # ~1.8s — a 3s goal banner now has ~90% catch probability vs ~25% before
    # the density increase. Still fits within the 50 min total budget.
    times = focused_frame_times(info.duration, focused or None,
                                every=adaptive_every, max_frames=2000)
    if not times:
        return []
